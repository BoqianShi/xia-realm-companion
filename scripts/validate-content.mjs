...
const entry = z.object({
  ...
  formula: z.object({
    ...
    distance: number, distanceUpgrade: number, action: z.enum(["main", "minor", "charge", "reaction"]),
  }).optional(),
  modifiers: z.array(modifier).optional(), effectModifiers: z.array(modifier).optional(),
}).passthrough();
const localUrl = z.string().refine(v => !v || /^\/modules\/[a-z0-9/_-]+\.(png|jpg|jpeg|webp|pdf|txt|docx|xlsx|json)$/i.test(v), "Use declared local /modules/ assets");
const npc = z.object({
  id, name: text, group: text, inner: text, note: text, source: text, url: localUrl,
...
const packSchema = z.object({
  ...
  encounters: z.array(z.object({
    ...
    cast: z.array(z.object({ id, npcId: id, name: text, count: number.int().min(0).max(30).nullable(), role: z.enum(["opening", "reserve", "support"]), side: z.enum(["ally", "enemy"]), note: text, optional: z.boolean(), maximum: number.nullable(), hp: number.nullable(), hpDelta: number.nullable(), track: z.boolean() })),
  })),
  starter: z.object({ title: text, description: text, background: text, build: z.object({}).passthrough(), learn: z.array(z.object({ id: text, level: number.int().min(1).max(4) })), equipment: z.array(text), activeWeapon: text, favorites: z.array(text) }),
...
export function validatePack(input) {
  ...
  for (const e of pack.encounters) {
    ...
    if (!m || e.pages.some(p => !m.pages[p - 1]) || e.cast.some(a => !m.npcs.some(n => n.id === a.npcId))) throw Error(`Unknown encounter module/NPC: ${e.id}`);
  ...
  for (const m of pack.modules) {
    for (const url of [m.cover, m.pdf, ...m.pages.map(p => p.image), ...m.assets.flatMap(a => [a.url, a.preview]), ...m.npcs.map(n => n.url)].filter(Boolean))
      if (!urls.has(url)) throw Error(`Undeclared asset: ${url}`);
  }
  return pack;
}
